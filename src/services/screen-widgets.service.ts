...
> {
  ...
  constructor(
    repo: ScreenWidgetsPgRepository,
  ...
  protected override guardCreate(
    data: typeof screenWidgets.$inferInsert,
    ...
    if (!isAllTenants) {
      ...
      if (!userTenantId) {
        return new ForbiddenDto("You must belong to a tenant to create a screen widget");
      }
      if (
        ...
        return new ForbiddenDto(
          "Only users with full tenant access can assign a screen widget to a different tenant",
        );
      }
      data.tenantId = userTenantId;
    }
  ...
  protected override guardUpdate(
    ...
    const accessErr = this.access?.guardResourceAccess("screenWidgets", existing);
